...
【personalized の問い方（観点）の分散 — 出力収束の防止・必須】
・personalized 5 問は、category が許可値内で重なっても、「問い方（観点）」は毎問できるだけ異ならせること。同じ問い方の質問を 3 問以上並べない。
・特に「その経験から何を学びましたか／どう成長しましたか」系の、抽象的な学び・成長を問う形に 5 問を寄せない。この形は最大 1 問までにとどめ、残りは下記の別観点に振り分ける。
・問い方の観点バンク（素材に自然に接続できるものを複数選ぶ。全部を使う必要はない）:
  - 動機の因果（なぜその選択に至ったか。きっかけと判断のつながり）
  - 判断の分岐点（迷った場面で何を基準に決めたか。他にどんな選択肢があり、なぜ捨てたか）
  - 具体場面の再現（その経験で最も印象的だった一場面を、状況→行動の順で語らせる）
  - 失敗と修正（うまくいかなかった点と、それにどう対応したか）
  - 他者・周囲との関係（誰とどう関わったか。対立・協力・役割の具体）
  - 数値・規模・期間（どのくらいの規模・期間・頻度だったか）
  - 比較（他の活動・他の大学・他の進路と比べて、なぜこれを選んだか）
  - 反対・批判への応答（自分の主張への想定反論にどう答えるか）
  - 前提・言葉の問い直し（本人が使った抽象語を、自分の定義で言い換えさせる）
  - 将来への中間ステップ（その経験から将来像に至るまでに、何が必要か）

【安全回答への収束防止】
・「リーダーシップ」「コミュニケーション力」「成長しました」のような、誰でも言える無難な美徳・抽象語に着地させる質問を避ける。
・抽象的な強みを問う場合は、必ず「それが実際に表れた具体的な一場面」を語らせる方向に変換する。
