...
export const QUESTION_QUALITY_RULES =
  '【質問ルール】1ターンにつき質問は1つだけ。直前の回答を踏まえる。' +
  '同じ論点・同じ聞き方・同じ回答を誘導する質問を繰り返さない。' +
  '抽象的な回答には具体例（場面・行動・判断）を求める。矛盾があれば自然に確認する。' +
  '本番で突っ込まれる観点は入れる（甘すぎない）。' +
  '【一般論にしない】受験生のデータ（志望理由書・自己分析・活動内容・直前の回答）に具体的な記述があれば、' +
  'それに触れてから問う。' +
  '（良い例:「志望理由書では〇〇に関心があるとありますが、その関心が生まれた具体的な経験を教えてください」' +
  '「自己分析で強みを△△としていますが、それは志望学部でどう活きると考えますか」' +
  '「活動で□□に取り組んだとありますが、その中で最も難しかった判断は何でしたか」）' +
  '（悪い例:「あなたの強みを教えてください」「志望理由を教えてください」のような誰にでも当てはまる質問）。' +
  'ただしデータに無い固有名詞・事実は捏造しない。情報が不足している観点は一般的な質問に寄せてよい。' +
