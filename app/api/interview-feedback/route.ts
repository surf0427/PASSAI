...
// M4: Vercel 実行時間上限。本 route は Opus + createTimeoutSignal(90_000) のため
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // B2: prompt に埋め込む自由記述の最大長ガード。AI call 前に弾く。
    const lengthCheck = checkMaxLengths([
      { label: '大学名', value: universityName, max: INPUT_MAX_LENGTHS.LABEL },
      { label: '学部・学科', value: facultyName, max: INPUT_MAX_LENGTHS.LABEL },
      { label: '志望理由', value: motivation, max: INPUT_MAX_LENGTHS.TEXT },
      {
        label: '質問と回答',
        value: questionsAndAnswers.flatMap((qa) => [qa.question, qa.answer]),
        max: INPUT_MAX_LENGTHS.ANSWERS_TOTAL,
      },
      { label: '質問一覧', value: questionsAsked, max: INPUT_MAX_LENGTHS.ANSWERS_TOTAL },
      { label: '回答内容', value: myAnswers, max: INPUT_MAX_LENGTHS.ANSWERS_TOTAL },
    ]);
    if (!lengthCheck.ok) {
...
${qaText}${heuristicSection ? `\n\n${heuristicSection}` : ''}`;
    ...
    try {
      ...
      await recordUsage({ userId, route: USAGE_ROUTE, model: MODEL, status: 'error' });
      captureRouteException(new Error('AI_FEEDBACK_PARSE_FAILED'), { route: ROUTE, feature: 'ai', status: 502 }, { status: 502, code: 'AI_FEEDBACK_PARSE_FAILED' });
      return NextResponse.json(
        {
