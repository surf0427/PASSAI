...
export default function SelfAnalysisPage() {
  ...
  async function handleSummarize() {
    ...
    if (
      ...
      // STEP-LOG-1: 完了済みログ履歴に snapshot を保存する。
    ...
    try {
      ...
      // STEP-LOG-1: 完了済みログ履歴に snapshot を保存する（API 成功経路）。
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-12">
      {/* confirm step（① 0から自己PRを書く の最初のページ）の上に、
          自己分析機能一覧 (/self-analysis hub) への戻り導線を追加。
          step が confirm 以外の時はステップ内戻る (handleBack) が表示されるため、
          多重表示を避けて confirm のみで出す。 */}
      ...
      {isMounted && step === 'summary' && summary && (
        <section>
          ...
          {/* 活動まとめの直後に診断タイプの分析コメントを挟み、
              ...
              currentHref は feature 識別子として "/self-analysis" を保つ（path 移動後も
              DiagnosisTypeCard 側の suppression rule は ctaHref と prop の一致で判定する
              ため、論理的に正しい）。*/}
          <div className="mt-8">
            <DiagnosisTypeCard
          ...
          <div className="mt-8 p-4 bg-gray-50 border border-gray-200 rounded-xl">
            ...
            {/* ①「0から自己PRを書く」フローを summary → self-pr 編集画面まで
                シームレスに繋ぐため、CTA は primary 1 本だけに絞る。
                過去一覧 / 結果ページへの遷移は hub の ② / ③ / ④ で扱う設計に集約済み。
                ?from=run を付けて /self-pr に着地させると、mount 効果側でリストを
                スキップし、現 canonical 入力 hash に対応する PR の編集画面を直接開く。 */}
            <Link
              href="/self-pr?from=run"
