...
function s5Invariants(): void {
  ...
  // ★ canonical context は route に存在するが shadow 専用である ★
  //   prompt へ載る studentContextSection は body.* だけから作られ、
  //   canonical の組み立てはその **後** に走り、出力は enum と件数だけを telemetry へ出す。
  const iSection = route.indexOf('const studentContext = buildTutorStudentContext(');
  const iShadowGate = route.indexOf('const shadowEnabled = isExamSpineShadowEnabled(');
  const iCanonical = route.indexOf('await buildCanonicalExamContext(');
  check('I5 prompt section は canonical より前に確定する',
    iSection !== -1 && iShadowGate !== -1 && iCanonical !== -1
    && iSection < iShadowGate && iShadowGate < iCanonical,
    `section=${iSection} gate=${iShadowGate} canonical=${iCanonical}`);
  // 呼び出しは 1 箇所だけで、それは shadow gate より後ろにある（import は数えない）。
  const calls: number[] = [];
  ...
  check('I5 その呼び出しは shadow gate の後ろ', calls[0] > iShadowGate, `call=${calls[0]} gate=${iShadowGate}`);
