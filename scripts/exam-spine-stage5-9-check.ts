...
function s8PriorStages(): void {
  ...
  // ★ 採番の健全性は「最大値が固定」ではなく **構造**で見る（S5-P12）★
  //   controlled consumer lineage の合流で E-S55〜E-S58 が加わり、最大値は動く。
  //   固定値で pin すると lineage 合流のたびに偽陽性になるため、
  //   presentation が E-S54 のままであること・重複が無いこと・連番であることを見る。
  const ids = [...dec.matchAll(/^## E-S([0-9]+)(?=[^0-9])/gm)].map((m) => Number(m[1]));
  eq('X6 decision ID に重複が無い', ids.length, new Set(ids).size);
  const sorted = [...ids].sort((a, b) => a - b);
  eq('X6 decision ID は 1..max の連番（欠番が無い）',
    sorted, Array.from({ length: sorted[sorted.length - 1] }, (_, i) => i + 1));
  check('X6 E-S54 より後ろの ID は presentation を上書きしていない',
    sorted[sorted.length - 1] >= 54);
  // presentation の番号そのものは動かない（controlled lineage 側が譲る / E-S38-3）。
  check('X6 presentation の decision 番号は E-S54 で固定',
    /^## E-S54 — `presentation`/m.test(dec));
