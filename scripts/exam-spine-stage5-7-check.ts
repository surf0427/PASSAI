...
async function t5Invariants(): Promise<void> {
  ...
  // ★ S5-P10: consumer path 全体を見る（stage5-2〜5-6 と同一の retarget）★
  //   本 lineage では body 経路の組み立てと prompt 合成が composeTutorPrompt へ抽出済みで、
  //   route 単体には呼び出しが無い。不変条件は「legacy の body 経路が今も組み立てられて
  //   いること」なので consumer path（route + composeTutorPrompt）を対象にし、
  //   import 行を除いた本体で **呼び出し形** を exact 名で検査する
  //   （`\w*` の緩い一致は改名 mutation を素通りさせる。stage5-6 で実測済み）。
  ...
  const routeCode = route
    ...
    .filter((l) => !/^\s*(\/\/|\*|\/\*)/.test(l))
    .join('\n');
  // ★ S5-P10: 実在する組み立て呼び出しのうち最も早いものへ anchor を広げた ★
