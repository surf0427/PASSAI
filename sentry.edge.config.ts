// Sentry edge (Edge runtime / middleware) 設定。
//
// instrumentation.ts の register() が NEXT_RUNTIME==='edge' のときに import する。
//
// 方針（最小安全構成）: server config と同一。Replay は client 専用のため含めない。
