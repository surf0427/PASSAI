...
| 5 | **selfPR drafts (selfPRs)** | `lib/selfPRStorage.ts` (`selfPRs`) | L | L | L | L | **Low** | **Wired (上り mirror); DB apply 待ち** | 5 | observability sink 稼働 | 自己 PR 一覧（save / list）。auth-scoped mirror `self_prs`（schema.sql §35–§37）。natural key = `(user_id, local_pr_id)`。backfill = 05C（AuthProvider, flag `'selfPRs'`）、dualWrite = 05D（`app/self-pr/page.tsx`, `propagateDelete=false`）。restore は delete resurrection 回避のため別 STEP（[self_prs_delete_safety_design.md](./self_prs_delete_safety_design.md)）|
...
| 12 | **statementReviewHistory** | `lib/statement/review/statementStorage.ts` (`statementReviewHistory`) | L | M | M | H | **Medium-High** | **Wired (上り mirror); release-pre scope complete (06A–06D); DB apply 待ち** | 10 | 観測安定 + StudentProfile / statement-review cache との因果関係文書化 | AI 添削履歴。cache (`statementReviewInputHash`) と分離して扱う。auth-scoped mirror table `statement_review_history`（schema.sql §38–§40, STEP-SUPABASE-COMPLETE-06A）。natural key = `(user_id, local_review_id)`、result = StatementResult jsonb。helper/repository = 06B、AuthProvider backfill（上りのみ・flag `'statementReviewHistory'`）= 06C、dualWrite（`app/statement/edit/page.tsx` の saveReviewHistory 2 経路直後・upsert のみ）= 06D。restore / delete 伝播は delete resurrection 回避のため 06E に分離（[statement_review_history_mirror_schema_preview.md](./statement_review_history_mirror_schema_preview.md)）|
